...
class PathSmoother:
    ...
    def _cubic_spline_interpolate(self, points: List[Tuple[float, float]]) -> List[TrajectoryPoint]:
        ...
        num_samples = max(int(distances[-1] / self.resolution), n * 3)
        ...
        for p in points:
            ...
            if len(result) > 0:
                ...
                if abs(dx) > 1e-6 or abs(dy) > 1e-6:
                    theta = math.atan2(dy, dx)
            result.append(TrajectoryPoint(x=p[0], y=p[1], theta=theta, v=0.0))
        return result
...
class TrajectoryTracker:
    ...
    def compute_cmd(
        ...
        linear_cmd = np.clip(linear_cmd, -0.4, 0.4)
    ...
    def _find_lookahead_target(
        ...
        for i in range(trajectory_length):
            ...
            if abs(dist - dynamic_lookahead) < best_score:
                best_score = abs(dist - dynamic_lookahead)
                best_idx = i
